...
import type { MessageWithReactions, Project, User, UserPresence } from '../types';
...
export default function Chat() {
  ...
  // Load members for messages
  useEffect(() => {
    const senderIds = new Set(messages.map((m) => m.senderId));
    const unknownIds = Array.from(senderIds).filter((id) => !members.has(id));
  ...
  // Load all members from all projects for @mentions
  useEffect(() => {
    ...
    if (allMemberIds.size > 0) {
      getUsersByIds(Array.from(allMemberIds)).then(setAllMembers);
    }
  }, [projects]);
  ...
  async function handleSendMessage(content: string) {
    ...
    await sendMessage(activeChannel.id, content, currentUser.uid, mentions);
                  ...
                  >
                    <div className="relative">
                      <div className="w-6 h-6 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-white text-xs font-medium">
                        {otherUser?.avatarUrl || otherUser?.displayName?.charAt(0).toUpperCase() || '?'}
                      </div>
                      <span
