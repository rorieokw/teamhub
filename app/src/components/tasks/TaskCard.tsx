...
export default function TaskCard({
  ...
  const blockers = task.blockedBy?.map((id) => allTasks.find((t) => t.id === id)).filter(Boolean) || [];
  const unblockedTasks = blockers.filter((b) => b && b.status !== 'done');
  const isBlocked = unblockedTasks.length > 0;
  ...
  return (
    <div className={`glass-card rounded-xl overflow-hidden transition-all group ${isDragging ? 'opacity-50 ring-2 ring-purple-500' : ''} ${isBlocked ? 'ring-1 ring-red-500/30' : ''}`}>
      ...
      {isBlocked && (
        <div className="bg-red-500/20 px-4 py-2 flex items-center gap-2 border-b border-red-500/20">
          ...
          <span className="text-red-400 text-xs font-medium">
            Blocked by {unblockedTasks.length} task{unblockedTasks.length > 1 ? 's' : ''}
          </span>
        </div>
      )}
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <h4 className={`font-medium text-sm flex-1 group-hover:text-purple-300 transition-colors ${isBlocked ? 'text-gray-400' : 'text-white'}`}>
          {task.title}
        </h4>
